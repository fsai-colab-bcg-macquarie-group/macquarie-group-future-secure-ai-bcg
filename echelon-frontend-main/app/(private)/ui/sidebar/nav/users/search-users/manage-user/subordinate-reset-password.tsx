...
export default function ResetPasswordOverlay({
  ...
  return (
    <div className="fixed top-0 left-0 z-[9000] flex min-h-screen min-w-screen place-items-center items-center justify-center border bg-[#e7eaeeda]">
...
function ResetPasswordForm({ callback }: { callback: () => void }) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validação dos dados do formulário com o schema
      ...
      // Esconde os campos de senha e confirmação após a validação
      setPassword(false)
  ...
  return (
    <form onSubmit={onSubmit}>
      <div className="anim_reveal_to_bottom relative w-[580px] bg-white p-6 pt-8">
        ...
        >
          <X className="stroke-1 text-[#5E6D79]" size={28} />
        </button>
        <section>
          ...
          <div className="relative w-fit">
            ...
            >
              {!errors.password &&
                ...
                (password ? (
                  <EyeClosed className="p-px text-[#5E6D79]" />
                ) : (
                  <Eye className="p-px text-[#5E6D79]" />
                ))}
            </div>
          </div>
